...
class UserBloc extends BlocBase {
  ...
  List<Map<String, dynamic>> _filter(String search){
    ...
    filteredUsers.retainWhere((user){
      return user["cpf"].toUpperCase().contains(search.toUpperCase());
    });
    return filteredUsers;
  }

  void _addUsersListener(){
    _firestore.collection("colaboradores").snapshots().listen((snapshot){
  ...
  void _subscribeToOrders(String uid){
    _users[uid]["subscription"] = _firestore.collection("colaboradores").document(uid)
        .collection("coletas").snapshots().listen((coletas) async {

          int numOrders = coletas.documents.length;

          double money = 0.0;

          for(DocumentSnapshot d in coletas.documents){
